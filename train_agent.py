...
def train_agent(restore_prior_from='/apdcephfs/private_jikewang/W4_reduce_RL/data/6w_Prior_fragment.ckpt',
    ...
    for step in range(n_steps):
        ...
        if step%50==0 and step!=0:
            torch.save(Agent.rnn.state_dict(), '/apdcephfs/private_jikewang/W4_reduce_RL/model/'+str(step)+'drd0.1.ckpt')
'''
    if not save_dir:
        save_dir = '/apdcephfs/private_jikewang/W4_reduce_RL/data/results/run_' + time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime())
    os.makedirs(save_dir)
    copyfile('/apdcephfs/private_jikewang/W4_reduce_RL/train_agent.py', os.path.join(save_dir, "/apdcephfs/private_jikewang/W4_reduce_RL/train_agent.py"))

    experience.print_memory(os.path.join(save_dir, "memory"))
    torch.save(Agent.rnn.state_dict(), os.path.join(save_dir, '/apdcephfs/private_jikewang/W4_reduce_RL/model/MC_MIT_301_Agent.ckpt'))
